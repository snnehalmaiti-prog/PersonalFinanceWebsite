...
const syncBlock = HTML.slice(HTML.indexOf("wf-cloud-synced"), HTML.indexOf("wf-cloud-synced") + 2500);
...
check(/el\.src = "script\.js\?v=__ASSET_VERSION__"/.test(HTML),
...
check(/setTimeout\(go, \d+\)/.test(HTML),
  "the injection needs a timeout fallback so a hung sync cannot strand the app");
check(/document\.dispatchEvent\(new Event\("wf-app-ready"\)\)/.test(HTML) &&
      /el\.onerror = function \(\) \{ document\.dispatchEvent/.test(HTML),
